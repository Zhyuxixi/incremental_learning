...
class Cifar100:
    def __init__(self):
        with open(r'WA_cifar100\cifar100\train', 'rb') as f:
            self.train = pickle.load(f, encoding='latin1')
        with open(r'WA_cifar100\cifar100\test', 'rb') as f:
    ...
    def initialize(self):
        ...
        for train_data, train_label in zip(self.train_data, self.train_labels):
            ...
            if train_label < 10:
                ...
                each_train_groups[train_label].append(
        ...
        assert len(val_groups[0]) == 500
        assert len(val_groups[1]) == 500
        assert len(val_groups[2]) == 500
        assert len(val_groups[3]) == 500
        assert len(val_groups[4]) == 500
        assert len(val_groups[5]) == 500
        assert len(val_groups[6]) == 500
        assert len(val_groups[7]) == 500
        assert len(val_groups[8]) == 500
        assert len(val_groups[9]) == 500

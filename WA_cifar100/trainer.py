...
class Trainer:
    ...
    def train(self, batch_size, epoches, lr, max_size):
        ...
        for inc_i in range(dataset.batch_num):
            ...
            ), lr=lr, momentum=0.9,  weight_decay=(2e-4/(inc_i+1)))
            ...
            test_acc = []
            ckp_name = 'WA_cifar100/checkpoint/{}_run_{}_iteration_{}_model.pth'.format(
                self.seen_cls-10, self.seen_cls, inc_i)
            if os.path.exists(ckp_name):
                ...
                if inc_i > 0:
                    self.model.eval()
                    weight_old = self.model.fc.weight.data[:self.seen_cls-10]
                    weight_new = self.model.fc.weight.data[self.seen_cls -
                                                           10:self.seen_cls]
                    Norm_weight_old = torch.sum(torch.norm(
                        weight_old, dim=1)) / torch.norm(weight_old, dim=1).shape[0]
                    Norm_weight_new = torch.sum(torch.norm(
                        weight_new, dim=1)) / torch.norm(weight_new, dim=1).shape[0]
                    lanbuda = Norm_weight_old/Norm_weight_new
                    self.model.fc.weight.data[:self.seen_cls-10] = weight_old
                    self.model.fc.weight.data[self.seen_cls -
                                              10:self.seen_cls] = weight_new * (lanbuda)
                    self.model.cuda()
